...
namespace Kiko.Repository
{
    public class UsersRepository
    {
        ...
        {
            var sql = @"select * from Users ;  Select * from Device ;  Select * from Device ; 
 ";
            var multi = db.QueryMultiple(sql);
            var _Userss = multi.Read<Users>().ToList();
            var _Devices = multi.Read<Device>().ToList();
            var _Devices = multi.Read<Device>().ToList();

            foreach (var item in _Userss)
            {
                item.Devices = _Devices.FindAll(x => x.UsersId == item.Id);
                item.Devices = _Devices.FindAll(x => x.UsersId == item.Id);
